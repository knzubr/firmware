...
void stack_init (void)
{
	...
	// Read IP Setup from EEPROM - changed to read from program memory.
	mlIP = pgm_read_dword_far(&mlIpEEP);
	mlNetmask = pgm_read_dword_far(&mlNetmaskEEP);
	
	
	//mlGateway = (mlGatewayEEP);
	//mlDNSserver = (mlDNSserverEEP);

	uint8_t i;
	for ( i = 0 ; i < 6 ; i++ )
	{
		mlMAC[i] = (enc28j60_config[ENC28J60_CONFIG_OFFSET_MAC + i*2]);
	}
