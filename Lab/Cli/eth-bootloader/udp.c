...
void udp (void)
{
	...
	UDP_packet = ( struct UDP_header *) &ethernetbuffer[ETH_HDR_LEN + ((IP_packet->IP_Version_Headerlen & 0x0f) * 4 )];
      // tu powinna byc	 sprawdzenie sumy kontrolnej!
	//
	uint16_t checsumC = UDP_packet->UDP_Checksum;
	

    UDP_packet = (struct UDP_header *)&ethernetbuffer[ETH_HDR_LEN + IP_HDR_LEN];
    IP_packet = (struct IP_header  *)&ethernetbuffer[ETH_HDR_LEN];
  
   //	   ethernetbuffer[100] = 1; //for simulate error in channel
	   UDP_packet->UDP_Checksum = 0;
  uint16_t checsumC1;
    //Berechnet Headerlänge und Addiert Pseudoheaderlänge 2XIP = 8
    uint16_t result16 = htons(IP_packet->IP_Totallenght) + 8;
    result16 = result16 - ((IP_packet->IP_Version_Headerlen & 0x0F) << 2);
	// will never exceed 0xffff, so word arithmetic should do
    uint32_t result32 = result16 + 0x09;
  
    //Routine berechnet die Checksumme
    checsumC1= Checksum_16 ((&IP_packet->IP_Version_Headerlen+12), result16, result32);
    //checsumC1 = result16;
    if((checsumC1 << 8) != (checsumC<<8) 
      ||
      (checsumC1)!= (checsumC) )
    return 0;
					...
					unsigned long dest_ip)
{
    ...
    //Berechnet Headerlänge und Addiert Pseudoheaderlänge 2XIP = 8
    result16 = htons(ip->IP_Totallenght) + 8; //  dodajemy osiem do calej dlugosci IP bo mamy 2x4octety,
    result16 = result16 - ((ip->IP_Version_Headerlen & 0x0F) << 2);//a potem odejmujemy dlugosc samego naglowka ip, 
	// will never exceed 0xffff, so word arithmetic should do //operacja sprawia ze mamy  wielkrotnosc 16 bitow.
    result32 = result16 + 0x09; // 9 BO 8+9 = 17 CZYLI PROT UDP tak jak przykazano ;]
