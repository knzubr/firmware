...
static cliExRes_t lightFunction(cmdState_t *state);
...
command_t __ATTR_PROGMEM__ cmdListNormal[] =
{
  ...
  {cmd_light,     cmd_help_light,     lightFunction},
...
static cliExRes_t lightFunction(cmdState_t *state)
{
  ...
  uint8_t nrSterownika = (cmdlineGetArgInt(1, state) & 0x3F);
  uint8_t nrDiody     = (cmdlineGetArgInt(2, state) & 0x01);
  uint8_t wartosc = 255;
  if (state->argc > 2)
    wartosc = cmdlineGetArgInt(3, state);

  fprintf_P(state->myStdInOut,   movingCurtainUpStr, nrSterownika, nrDiody+1);
  if ((wartosc > 0) && (wartosc <=100))
    fprintf_P(state->myStdInOut, movingCurtainPosStr, wartosc);
